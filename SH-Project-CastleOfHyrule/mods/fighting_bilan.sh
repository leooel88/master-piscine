...
if [[ $playerHp -le 0 ]] ; then
    ...
    if [[ $revive -eq 0 ]] ; then
        ...
        source ./replace_str_characters.sh "1" "100" "1" "1"
    ...
    echo -e "\n${RED}${BOLD}$enemieName${NC} died!\n\n"
    echo -e "\nYou are so strong ${BOLD}$playerName${NC} !\n\n"
    ...
    fixedDamageToPlayer=$(($damageToPlayer-1))
    if [[ $fixedDamageToPlayer -le 0 ]] ; then
        damageToPlayer=0
        echo -e "\n$enemieName attacked and dealt "$damageToPlayer" damages!\n"
        return
    else
        echo -e "\n$enemieName attacked and dealt "$fixedDamageToPlayer" damages!\n"
        return
    fi
fi
